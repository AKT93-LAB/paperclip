...
**Step 6 — Understand context (token-safe).** `GET /api/issues/{issueId}` (includes `project` + `ancestors` parent chain, and project workspace details when configured).

Comments can get huge. Default to **bounded reads**:

- If `PAPERCLIP_WAKE_COMMENT_ID` is set: fetch that exact comment first via:
  - `GET /api/issues/{issueId}/comments/{commentId}`
- Then fetch only the **latest** comments for surrounding context:
  - `GET /api/issues/{issueId}/comments?limit=50`

Only fetch the full thread (no `limit`) when you truly need deep history (rare).
